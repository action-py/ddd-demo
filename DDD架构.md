...
DDD推荐的架构模式，这些架构模式用于指导服务内的具体实现，对于服务内的逻辑分层，职能角色，依赖关系都有现实的指导意义。
...
![传统的DDD分层-来自《实现领域驱动设计》](image/traditional-layer.png)

在传统的DDD分层架构中，基础层是被其它层所共同依赖的，它处于最底层，这可能导致重心偏移（想象一下在Service依赖DAO的场景),然而在DDD中领域层才是核心，因此要改变这种依赖。
...
![改进后的DDD分层-来自《实现领域驱动设计》](image/dip-traditional-layer.png)
...
![将不同的接入适配到内部接口](image/adapter.png)
